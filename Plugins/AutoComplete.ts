    ...
    constructor(input:any){
     ...
     if (targetControl.is('[data-typeahead-enabled=true]')) return;
        ...
        var dataSource = (query, callback) => {
            ...
            var data = this.getPostData(targetControl);
        ...
        input.data("selected-text", "").on('input', clearValue).on('blur', itemBlured).on('typeahead:selected', itemSelected).typeahead({ minLength: 0 }
}
